...
QString Database::import_message::description (bool extended) const
{
...
unsigned int Database::get_last_errno () const
{
	if (mysql)
...
void Database::list_required_tables (QStringList &tables)
{
...
void Database::list_required_writeable_tables (QStringList &tables)
{
	tables.push_back ("flug_temp");
	tables.push_back ("flugzeug_temp");
	tables.push_back ("person_temp");
...
dbTable Database::get_table_information (const QString table_name, bool resolve_like) const
	/*
...
// Database management
int Database::grant (QString permissions, QString target, QString user, QString password)
{
...
int Database::grant (QString permissions, QString target, QString user)
{
	return grant (permissions, target, user, "");
}

int Database::create_database (QString name, bool force)
{
...
int Database::create_table (const dbTable &tab, bool force)
{
	QString query=tab.mysql_create_query (force);
	return execute_query (query);
}

int Database::list_tables (QStringList &tables)
{
	return list_strings_query ("SHOW TABLES", 0, tables);
}

int Database::list_column_names (QStringList &names, QString table)
{
...
int Database::list_columns (QList<DbColumn> &columns, QString table)
{
	MYSQL_RES *result;
...
int Database::modify_column (const QString &table, const DbColumn &column)
{
...
// Generic functions
char *Database::named_field_value (MYSQL_ROW &row, int num_fields, MYSQL_FIELD *fields, const char *name)
...
QString Database::escape (QString text)
{
	if (connected ())
	{
...
QString Database::fixed_table_name (db_object_type type)
{
	switch (type)
	{
...
QString Database::editable_table_name (db_object_type type)
{
	switch (type)
	{
...
QString Database::default_sort_column (db_object_type type)
{
	switch (type)
	{
...
QString Database::query_value_list (db_object_type type, void *object)
{
...
int Database::row_to_object (db_object_type otype, void *object, MYSQL_ROW row, int num_fields, MYSQL_FIELD *fields)
{
...
long long int Database::row_to_number (MYSQL_ROW row, int num_fields, MYSQL_FIELD *fields, const char *field_name)
{
...
int Database::copy_object (db_object_type type, void *target, void *source)
{
...
void *Database::new_object (db_object_type type)
{
	switch (type)
	{
...
int Database::free_object (db_object_type type, void *object)
{
	switch (type)
	{
...
QString Database::object_name (db_object_type type, bool plural)
{
	switch (type)
	{
...
// Bitmask types
bool Database::status_gestartet (unsigned int status)
{
	return (status & STATUS_GESTARTET);
}

unsigned int Database::make_status (bool gestartet, bool gelandet, bool sfz_gelandet)
{
	int r=0;
...
bool Database::status_gelandet (unsigned int status)
{
	return (status & STATUS_GELANDET);
}

bool Database::status_sfz_gelandet (unsigned int status)
{
...
int Database::execute_query (MYSQL_RES **result, QString query_string, bool retrieve_immediately)
...
QString Database::make_query (query_type_t query_type, db_object_type object_type, QString condition, QString columns, bool add_column_editable, bool distinct, QString sort_column, QString group_column, bool editable_flag)
	/*
...
QString Database::make_condition (Condition c)
...
bool Database::object_exists_in (const QString &table, const db_id &id)
...
void Database::object_delete_from (const QString &table, const db_id &id)
	throw (ex_legacy_error)
	/*
...
void Database::object_copy (const QString &source, const QString &target, db_id id)
	throw (ex_legacy_error)
	/*
...
// Editable
void Database::make_editable (db_object_type type, int id, bool editable)
...
// Writing
db_id Database::write_object (db_object_type type, void *object)
{
...
// Deletion
int Database::delete_object (db_object_type otype, db_id id)
{
...
// Existance
int Database::num_results_query (QString query)
...
int Database::rows_exist_query (QString query)
{
...
int Database::object_exists (db_object_type type, db_id id)
{
...
int Database::flight_exists (db_id id)
{
	return object_exists (ot_flight, id);
}

int Database::plane_exists (db_id id)
{
	return object_exists (ot_plane, id);
}

int Database::person_exists (db_id id)
{
...
int Database::result_to_string_list (QStringList &strings, MYSQL_RES *result, const char *field_name)
{
	// TODO code duplication with result_to_list
	if (!result) return db_err_parameter_error;
...
int Database::result_to_string_list (QStringList &strings, MYSQL_RES *result, const unsigned int field_num)
{
	// TODO code duplication with result_to_list
...
int Database::list_strings (db_object_type type, QString field_name, QStringList &strings, Condition c, QString custom_query)
{
...
int Database::list_strings_query (const QString query, const QString field_name, QStringList &strings)
{
...
int Database::list_airfields (QStringList &airfields)
{
...
int Database::list_first_names (QStringList &names, QString last_name)
{
...
int Database::list_last_names (QStringList &names, QString first_name)
{
...
// Counting
long long int Database::count_objects (db_object_type type, Condition c)
{
	// TODO code duplication with list_objects
	if (!connected ()) return db_err_not_connected;
...
long long int Database::count_planes (Condition c)
{
	return count_objects (ot_plane, c);
}

long long int Database::count_persons (Condition c)
{
...
bool Database::person_has_flight (db_id id)
{
	return object_has_flight (ot_person, id);
}

bool Database::person_has_user (db_id id)
{
...
bool Database::plane_has_flight (db_id id)
{
	return object_has_flight (ot_plane, id);
}

bool Database::person_used (db_id id)
{
	return (person_has_flight (id) || person_has_user (id));
}

bool Database::plane_used (db_id id)
{
...
int Database::count_startart ()
{
	return startarten.count ();
}

bool Database::add_startart_to_list (LaunchType *sa)
{
...
int Database::get_startart_by_type (LaunchType *startart, startart_type sat)
{
	bool found=false;
...
int Database::get_towplane (Plane *towplane, const LaunchType &startart, const db_id towplane_id)
{
...
void Database::merge_person (db_id correct_id, db_id wrong_id)
...
db_id Database::import_identify (const Person &p, QList<import_message> *non_fatal_messages)
...
db_id Database::import_person (const Person &person)
	throw (ex_not_connected, ex_legacy_error, ex_operation_failed)
{
	if (!connected ()) throw ex_not_connected ();
