...
DbColumn::DbColumn (const QString &_name, const enum_field_types _type, const unsigned int _length, const QString &_def)
	:name (_name), type (_type), length (_length), def (_def)
{
	init_flags ();
}

DbColumn::DbColumn (const MYSQL_FIELD &f)
{
...
QString DbColumn::mysql_spec () const
{
	QString r;
