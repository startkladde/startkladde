...
QString LatexDocument::package::make_use_clause () const
{
	QString r;
	r.append ("\\usepackage");
...
LatexDocument::package::package (const QString &_name, const QString &opt0, const QString &opt1, const QString &opt2, const QString &opt3)
{
	name=_name;
...
void LatexDocument::write_header ()
{
...
void LatexDocument::write_footer ()
{
	doc << std::endl;
	doc << "\\end{document}" << std::endl;
}

void LatexDocument::add_package (const package &p)
{
...
QString LatexDocument::get_string ()
{
...
LatexDocument &LatexDocument::write (const Table &tab, const TableRow &header, const QList<float> &widths)
{
	// This writes a Table. It is a bit q'n'd, in lack of a proper data
	// structure. Only rows for which widths exist are written.

	TableRow::const_iterator header_end=header.end ();
	QList<float>::const_iterator widths_end=widths.end ();
	TableRow::const_iterator column;
	...
	{
		int col=0;
		TableRow::const_iterator row_end=(*row).end ();
...
LatexDocument &LatexDocument::write_text (const QString &text)
{
	body << latex_escape (text);
	return *this;
}

LatexDocument &LatexDocument::start_section (const QString &caption)
{
	body << "\\section{" << latex_escape (caption) << "}" << std::endl;
	return *this;
}

LatexDocument &LatexDocument::write_empty_line ()
{
	body << std::endl;
	return *this;
}
...
// TODO auslagern
void LatexDocument::execute_command (const QString &command, const QString &location) throw (ex_generate_error)
{
...
QString LatexDocument::make_pdf ()
	throw (SkException)
{
	// Make a temporary directory
	TempDir dir ("sk_web.latex.");
